...
describe('일정 관리 애플리케이션 통합 테스트', () => {
  describe('일정 CRUD 및 기본 기능', () => {
    ...
    test('일정을 삭제하고 더 이상 조회되지 않는지 확인한다', async () => {
      ...
      expect(
        within(eventList).queryByText('삭제할 회의')
  ...
  describe('일정 뷰 및 필터링', () => {
    ...
    test('주별 뷰에 일정이 정확히 표시되는지 확인한다', async () => {
      ...
      const fullDate = `${date.getFullYear()}-${fillZero(
        date.getMonth() + 1
      ...
      expect(
        within(eventList).getByText(`${fullDate} 09:00 - 10:00`)
      ).toBeInTheDocument();
    ...
    test('월별 뷰에 일정이 정확히 표시되는지 확인한다', async () => {
      ...
      const monthEventTitleEl = await within(monthView).findByText(
        '이번달 팀 회의'
      );

      expect(monthEventTitleEl).toHaveTextContent('이번달 팀 회의');
      expect(
        within(monthEventTitleEl.closest('td')!).queryByText('1')
      ).toBeInTheDocument();
  ...
  describe('검색 기능', () => {
    ...
    test('제목으로 일정을 검색하고 정확한 결과가 반환되는지 확인한다', async () => {
      ...
      expect(
        within(eventList).getByText('2024-07-15 09:00 - 10:00')
      ).toBeInTheDocument();
      expect(
        within(eventList).queryByText('프로젝트 계획')
      ).not.toBeInTheDocument();
    });
